...
from isic.core.serializers import IsicIdListSerializer, SearchQuerySerializer
...
@shared_task
def populate_collection_from_isic_ids_task(collection_pk: int, user_pk: int, isic_ids: str) -> None:
    user = User.objects.get(pk=user_pk)
    collection = Collection.objects.get(pk=collection_pk)
    serializer = IsicIdListSerializer(data=isic_ids, context={'user': user})
    serializer.is_valid(raise_exception=True)
    collection.images.add(*serializer.to_queryset())
