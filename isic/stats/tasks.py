...
def _get_analytics_client():
    ...
    credentials = service_account.Credentials.from_service_account_info(settings.ISIC_GOOGLE_API_JSON_KEY)
...
def _cdn_log_objects(s3, after: str | None) -> Iterable[dict]:
    ...
    pages = s3.get_paginator("list_objects_v2").paginate(Bucket=settings.ISIC_CDN_LOG_BUCKET, **kwargs)
    for page in pages:
