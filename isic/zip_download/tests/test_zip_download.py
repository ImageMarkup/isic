...
def test_zip_download_listing_urls(authenticated_client, zip_basic_auth, image_factory, user):
    ...
    expected_public_url = f"https://{storages['sponsored'].bucket_name}.s3.amazonaws.com/{public_image.accession.sponsored_blob}"
    expected_private_url = (
        f"https://{default_storage.bucket_name}.s3.amazonaws.com/{private_image.accession.blob}"
    )
