...
def _zip_file_listing_generator(
    ...
    if settings.ZIP_DOWNLOAD_WILDCARD_URLS:
        ...
        for image in qs.values(
            "accession__blob", "accession__public_blob", "public", "isic_id"
        ).iterator():
            if image["public"]:
                url = storages["sponsored"].url(name=image["accession__public_blob"])
                zip_path = (
                    f"{image['isic_id']}.{extension_from_str(image['accession__public_blob'])}"
                )
            else:
