...
def _zip_file_listing_generator(qs: QuerySet[Image], token: str) -> Generator[dict[str, str]]:
    ...
    if hasattr(default_storage, "base_url"):
        default_url_prefix = default_storage.base_url.rstrip("/")
    else:
        default_url_prefix = f"https://{default_storage.bucket_name}.s3.amazonaws.com"

    if hasattr(storages["sponsored"], "base_url"):
        sponsored_url_prefix = storages["sponsored"].base_url.rstrip("/")
    else:
        sponsored_url_prefix = f"https://{storages['sponsored'].bucket_name}.s3.amazonaws.com"
    ...
    for image in (
        ...
        if image["public"]:
            url = f"{sponsored_url_prefix}/{image['accession__sponsored_blob']}"
            zip_path = (
                f"{image['isic_id']}.{extension_from_str(image['accession__sponsored_blob'])}"
            )
        else:
            url = f"{default_url_prefix}/{image['accession__blob']}"
